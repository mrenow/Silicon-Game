...
public class DataDisplay extends ScrollPane{
	...
	public LinkedList<Oscilliscope> scopes = new LinkedList<Oscilliscope>();
	public DataDisplay(float x, float y, float w, float h, Container p) {
		super(x, y, w, h, ScrollPane.SCROLL_X, p);
	}
	
	public void addScope(int x, int y) {
		if(numscopes < MAX_SCOPES) {
			scopes.add(new Oscilliscope(x,y,this));
			numscopes++;
			updateScopes();
		} else {
	...
	public void removeScope(Oscilliscope o) {
		if(scopes.remove(o)) {
			this.remove(o);
			numscopes--;
			updateScopes();
